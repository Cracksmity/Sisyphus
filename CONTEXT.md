...
- **Migraciones:** no hay Alembic; se usa parcheo incremental en `ensure_sqlite_schema()`
...
- Caché de respuesta por hash de payload final (mensaje+prompt+modelo+max_tokens)
...
- `rate_limit` y cachés son in-memory (no confiables entre procesos).
- Cambios de esquema dependen de `ensure_sqlite_schema()` + `create_all`.
